...
// Default settings
// CT Custom - Changed useAddressEnteredText <em> to <span>
ContactDataServices.defaults = {
	...
	useAddressEnteredText: "<span>Enter address manually</span>",
	useSpinner: false,
	language: "en",
	addressLineLabels: [
		"addressLine1",
...
ContactDataServices.mergeDefaultOptions = function(customOptions){
	...
	instance.useAddressEnteredText = instance.useAddressEnteredText;
...
ContactDataServices.address = function(customOptions){
	...
	instance.search = function(event){
		...
		if(instance.canSearch()){
			...
			// CT Custom - Prevents fields from being cleared when conducting a search
			// Moved this action to when an address from the picklist is selected
	...
	instance.canSearch = function(){
		...
		return (instance.enabled &&
				...
				// CT Custom - Search is always active with below line commented out	
				// && instance.hasSearchInputBeenReset === true                
		);
	};
	...
	instance.picklist = {
		...
		useAddressEntered: {
			...
			create: function(){
				var item = {
					suggestion: '<span>'+instance.useAddressEnteredText+'</span>',
					format: ""
				};
				
				//CT Custom - If there is a search again link then remove it
		...
		pick: function(item){
			...
			// CT Custom - Clear search after selecting an address from picklist; required due to not clearing search on new search
			instance.result.hide();
	...
	instance.result = {
		...
		updateAddressLine: function(key, addressLineObject){
			...
			if (instance.elements[key]){
				...
				// CT Custom - Address values were being appended to current address values here
				// This could cause a bug if an address was selected multiple times in quick succession so I removed it
				// It might be we don't need this function (or I've broken something)
		...
		// CT Custom - Check link doesn't already exist
		createSearchAgainLink: function(){
			if(instance.searchAgain.visible && $(".search-again-link").length === 0){
	...
	instance.handleError = {
		...
		showSubmitButton: function(){
			if($( ".qas-enter-manually" ).length === 0){
				var button = document.createElement("button");
				//CT Custom - Style error button
				button.innerText = instance.useAddressEnteredText;
				button.className = "button button--secondary button--fullwidth qas-enter-manually"
